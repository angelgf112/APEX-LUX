...
export class AdminGraficasComponent implements OnInit {
  ...
  ngOnInit(): void {
    const cotizacionesRef = collection(
      this.firestore,
      'cotizaciones'
    ) as CollectionReference<DocumentData>;

    collectionData(cotizacionesRef, { idField: 'id' })
      ...
      .subscribe((data) => {
        ...
        const fechas = Array.from(fechaMap.entries()).sort(
          (a, b) => a[0] - b[0]
        );
        ...
        this.chartDataFechas.datasets[0].data = fechas.map(
          ([, count]) => count
        );
        ...
        const tipos = Array.from(financiamientoMap.entries()).sort((a, b) =>
          a[0].localeCompare(b[0])
        );
        this.chartDataFinanciamiento.labels = tipos.map(([tipo]) => tipo);
        this.chartDataFinanciamiento.datasets[0].data = tipos.map(
          ([, count]) => count
        );
        ...
        const vehiculos = Array.from(vehiculoMap.entries()).sort((a, b) =>
          a[0].localeCompare(b[0])
        );
        this.chartDataVehiculos.labels = vehiculos.map(([id]) => id);
        this.chartDataVehiculos.datasets[0].data = vehiculos.map(
          ([, count]) => count
        );
